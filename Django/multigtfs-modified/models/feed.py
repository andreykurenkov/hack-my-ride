            ...
            for klass in gtfs_order:
                for f in filelist:
                    if f.endswith(klass._filename):
                        ...
                        if count==1 and klass==Agency:
                            setagency=Agency.objects.latest('id')
                        if setagency!=None and klass==Route:
                            #Enforce that no agency_id is null
                            routes = Route.objects.order_by('-id').exclude(agency__isnull=False)
                            routes.update(agency=setagency)
                        end_time = time.time()
                        logger.info(
